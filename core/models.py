...
def get_file_path(instance, filename):
    ...
    filename = "%s.%s" % (
        instance.slug if hasattr(instance, "slug") else timezone.now().timestamp(),
        ext,
    )
...
class UserProfile(models.Model):
    ...
    total_kpi_points = models.IntegerField(
        default=0, db_index=True, verbose_name="Tổng điểm KPI"
    )
    level_rank = models.CharField(
        max_length=50, default="Intern", verbose_name="Cấp bậc"
    )
...
class PointHistory(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="point_histories"
    )
...
class DocumentResource(BaseContent):
    ...
    file_upload = models.FileField(
        upload_to=get_file_path, verbose_name="File đính kèm"
    )
    file_type = models.CharField(max_length=10, choices=FILE_TYPES, default="DOC")
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, verbose_name="Người upload"
    )
...
class Restaurant(models.Model):
    ...
    image = models.ImageField(
        upload_to="restaurants/", verbose_name="Ảnh đại diện", null=True, blank=True
    )
    ...
    category = models.CharField(
        max_length=100, default="Món ngon", verbose_name="Loại hình"
    )
...
class HealthExercise(models.Model):
    ...
    code = models.CharField(
        max_length=20, choices=EXERCISE_CHOICES, unique=True, verbose_name="Mã bài tập"
    )
...
class Confession(BaseContent):
    ...
    def total_reactions(self):
        return self.loves_count + self.angry_count
...
class Reaction(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    post = models.ForeignKey(
        Confession, on_delete=models.CASCADE, related_name="reactions"
    )
...
class PostReport(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    post = models.ForeignKey(
        Confession, on_delete=models.CASCADE, related_name="reports"
    )
...
class HiddenPost(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="hidden_posts"
    )
...
class Product(models.Model):
    ...
    name = models.CharField(max_length=200,default="Sản Phẩm", verbose_name="Tên sản phẩm")
    
    # Dùng trường này để hiển thị giá tiền (VD: "159.000đ") thay vì điểm KPI
    price_display = models.CharField(max_length=50, default="Liên hệ", verbose_name="Giá hiển thị")
    created_at = models.DateTimeField(auto_now_add=True, null=True, blank=True)
    # Quan trọng: Trường này để lưu link Shopee/Lazada
    affiliate_url = models.URLField(max_length=500,default="https://shopee.com", verbose_name="Link Affiliate")
...
class DailyQuote(models.Model):
    ...
    def __str__(self):
        return f"[{self.time_category}] {self.content[:20]}"
